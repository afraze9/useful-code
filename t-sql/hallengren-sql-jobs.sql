...
--- WardyIT Solutions ---
DECLARE @CleanupTimeSystem nvarchar(max)
DECLARE @CleanupTimeDiff nvarchar(max)
DECLARE @CleanupTimeFull nvarchar(max)
DECLARE @CleanupTimeLog nvarchar(max)
DECLARE @SystemTime nvarchar(max)
DECLARE @FullDay nvarchar(max)
DECLARE @FullTime nvarchar(max)
DECLARE @DiffTime nvarchar(max)
DECLARE @LogTime nvarchar(max)
DECLARE @LogInterval nvarchar(max)
DECLARE @LogFrequency nvarchar(max)
DECLARE @IntegrityDay nvarchar(max)
DECLARE @IntegrityTime nvarchar(max)
DECLARE @IndexDay nvarchar(max)
DECLARE @IndexTime nvarchar(max)

DECLARE @ScriptVersion VARCHAR(5)
DECLARE @UseDiffs nvarchar(max)

SET @ScriptVersion = 'v1.9'

DECLARE @ToCompressBackups nvarchar(max)
DECLARE @Charshort nvarchar(4)
DECLARE @ErrorMessage nvarchar(max)
---------- END ----------
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------

                      Pre-Execution Variables - Must be set Prior to Execution to prevent Error

-----------------------------------------------------------------------------------------------------------------------------------------------------------------*/

SET @CreateJobs          = 'Y'        -- Specify whether jobs should be created. Default Value is Y
SET @BackupDirectory     = N'C:\Backups'       -- Specify the backup root directory.
SET @CleanupTimeSystem   = '120'        -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
SET @CleanupTimeFull     = '335'        -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
SET @CleanupTimeDiff     = '335'        -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
SET @CleanupTimeLog      = '335'        -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
SET @ToCompressBackups   = 'Y'       -- Should backup compression be enabled?
SET @OutputFileDirectory = NULL        -- Specify the output file directory. Set to NULL if the SQL Server error log directory should be used.
SET @LogToTable          = 'Y'        -- Log commands to a table. Default Value is Y
SET @UseDiffs            = 'Y'       -- Determines if the FULL Schedule should be created for DIFF Backups or not -- Accepted Values Y or N


---------Defining Job Schedules-----------
SET @SystemTime         = '223000'        --Set in 24 hour time for when the System Backups should execute. Example 220000 is 10pm

SET @FullDay            = 'Saturday'        --Only Needs to be Set if UseDiffs is set to Y. Determines what Day the Full should run. Set with Weekday ex Monday
SET @FullTime           = '090000'        --Set in 24 hour time for when the Full Backups should execute. Example 220000 is 10pm

SET @DiffTime           = '090000'        --Set in 24 hour time for when the DIFF Backups should execute. Example 220000 is 10pm

SET @LogTime            = '000000'        --Set in 24 hour time for when the Logs Backups should start executing. Example 220000 is 10pm
SET @LogInterval        = 'Minutes'        --Determines what interval the Log runs at. Set with Seconds, Minutes of Hours
SET @LogFrequency       = '15'        --Determines the frequency at which they occur based on the interval. Example if Inteval is 0x4 and frequency is 15 then logs occur every 15 minutes

SET @IntegrityDay       = 'Saturday'        --Determines what Day the Integrity Checks should run. Set with Weekday ex Monday
SET @IntegrityTime      = '010000'        --Set in 24 hour time for when the Integrity Checks should execute. Example 220000 is 10pm     

SET @IndexDay           = 'Sunday'        --Determines what Day the Integrity Checks should run.Set with Weekday ex Monday
SET @IndexTime          = '010000'        --Set in 24 hour time for when the Index Maintenance . Example 220000 is 10pm  
...
----Wardy IT Solutions----
IF @CreateJobs = 'Configure Me'
BEGIN
  RAISERROR('You need to set if Jobs should be created to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @BackupDirectory = 'Configure Me'
BEGIN
  RAISERROR('You need to set the backup directory to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @CleanupTimeSystem = 'Configure Me'
BEGIN
  RAISERROR('You need to set the system clean up time to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @CleanupTimeFull = 'Configure Me'
BEGIN
  RAISERROR('You need to set the full clean up time to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @UseDiffs = 'Y' and @CleanupTimeDiff = 'Configure Me'
BEGIN
  RAISERROR('You need to set the diff clean up time to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @CleanupTimeLog = 'Configure Me'
BEGIN
  RAISERROR('You need to set the log clean up time to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @ToCompressBackups = 'Configure Me'
BEGIN
  RAISERROR('You need to set if compression should be used to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @OutputFileDirectory = 'Configure Me'
BEGIN
  RAISERROR('You need to set the output directory to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @LogToTable = 'Configure Me'
BEGIN
  RAISERROR('You need to set if log to a table should be used to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @UseDiffs = 'Configure Me'
BEGIN
  RAISERROR('You need to set if DIFF Schedule is required to install the solution.',16,1)
  SET @Error = @@ERROR
END

--Schedule Checking--

IF @UseDiffs = 'Y' and @FullDay = 'Configure Me'
BEGIN
  RAISERROR('You need to set which day the Full backup should run when using Diffs to install the solution.',16,1)
  SET @Error = @@ERROR
END

IF @UseDiffs = 'Y' and @FullDay not in ('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday') 
BEGIN
  RAISERROR('@FullDay must be set to a day of the week to install the solution ',16,1)
  SET @Error = @@ERROR
END

IF ISNUMERIC(@LogTime)<> 1
BEGIN
  RAISERROR('Schedule Times must be set in 24hr time and must be in the following format 000000',16,1)
  SET @Error = @@ERROR
END

IF ISNUMERIC(@LogFrequency)<> 1
BEGIN
  RAISERROR('Log Frequency must be set to a numeric value',16,1)
  SET @Error = @@ERROR
END

IF LEN(@LogTime) <> 6
BEGIN
SET @Charshort = (select SUM(6-LEN(@LogTime)))
SET @ErrorMessage = 'Schedule Times must be set in 24hr time and must be in the following format 000000, your current value is ' + @LogTime + ' and is ' + @Charshort +' character short'
  RAISERROR(@ErrorMessage,16,1) with nowait
  SET @Error = @@ERROR
END

IF @UseDiffs = 'Y' and ISNUMERIC(@DiffTime)<> 1
BEGIN
  RAISERROR('Schedule Times must be set in 24hr time and must be in the following format 000000',16,1)
  SET @Error = @@ERROR
END

IF @UseDiffs = 'Y' and LEN(@DiffTime) <> 6
BEGIN
SET @Charshort = (select SUM(6-LEN(@DiffTime)))
SET @ErrorMessage = 'Schedule Times must be set in 24hr time and must be in the following format 000000, your current value is ' + @DiffTime + ' and is ' + @Charshort +' character short'
  RAISERROR(@ErrorMessage,16,1) with nowait
  SET @Error = @@ERROR
END


IF ISNUMERIC(@FullTime)<> 1
BEGIN
  RAISERROR('Schedule Times must be set in 24hr time and must be in the following format 000000',16,1)
  SET @Error = @@ERROR
END
...
IF LEN(@FullTime) <> 6
BEGIN
SET @Charshort = (select SUM(6-LEN(@FullTime)))
SET @ErrorMessage = 'Schedule Times must be set in 24hr time and must be in the following format 000000, your current value is ' + @FullTime + ' and is ' + @Charshort +' character short'
  RAISERROR(@ErrorMessage,16,1) with nowait
  SET @Error = @@ERROR
END

IF ISNUMERIC(@SystemTime)<> 1
BEGIN
  RAISERROR('Schedule Times must be set in 24hr time and must be in the following format 000000',16,1)
  SET @Error = @@ERROR
END
...
IF LEN(@SystemTime) <> 6
BEGIN
SET @Charshort = (select SUM(6-LEN(@SystemTime)))
SET @ErrorMessage = 'Schedule Times must be set in 24hr time and must be in the following format 000000, your current value is ' + @SystemTime + ' and is ' + @Charshort +' character short'
  RAISERROR(@ErrorMessage,16,1) with nowait
  SET @Error = @@ERROR
END

IF ISNUMERIC(@IntegrityTime)<> 1
BEGIN
  RAISERROR('Schedule Times must be set in 24hr time and must be in the following format 000000',16,1)
  SET @Error = @@ERROR
END

IF LEN(@IntegrityTime) <> 6
BEGIN
SET @Charshort = (select SUM(6-LEN(@IntegrityTime)))
SET @ErrorMessage = 'Schedule Times must be set in 24hr time and must be in the following format 000000, your current value is ' + @IntegrityTime + ' and is ' + @Charshort +' character short'
  RAISERROR(@ErrorMessage,16,1) with nowait
  SET @Error = @@ERROR
END

IF ISNUMERIC(@IndexTime)<> 1
BEGIN
  RAISERROR('Schedule Times must be set in 24hr time and must be in the following format 000000',16,1)
  SET @Error = @@ERROR
END
...
IF LEN(@IndexTime) <> 6
BEGIN
SET @Charshort = (select SUM(6-LEN(@IndexTime)))
SET @ErrorMessage = 'Schedule Times must be set in 24hr time and must be in the following format 000000, your current value is ' + @IndexTime + ' and is ' + @Charshort +' character short'
  RAISERROR(@ErrorMessage,16,1) with nowait
  SET @Error = @@ERROR
END

IF @LogInterval not in ('Seconds','Minutes','Hours')
BEGIN
  RAISERROR('@LogInterval must be either Seconds, Minutes or Hours to install the solution ',16,1)
  SET @Error = @@ERROR
END

IF @IntegrityDay not in ('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday') 
BEGIN
  RAISERROR('@FullDay must be set to a day of the week to install the solution ',16,1)
  SET @Error = @@ERROR
END

IF @IndexDay not in ('Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday') 
BEGIN
  RAISERROR('@FullDay must be set to a day of the week to install the solution ',16,1)
  SET @Error = @@ERROR
END

IF @LogInterval = 'Seconds' and @LogFrequency < 10
BEGIN
  RAISERROR ('@LogFrequency must be greater or equal to 10 when backing logs up every X seconds. Adjust this to install the solution',16,1)
  SET @Error = @@ERROR
END
...
INSERT INTO #Config ([Name], [Value])
VALUES('CleanupTimeSystem', @CleanupTimeSystem)

INSERT INTO #Config ([Name], [Value])
VALUES('CleanupTimeFull', @CleanupTimeFull)

INSERT INTO #Config ([Name], [Value])
VALUES('CleanupTimeDiff', @CleanupTimeDiff)

INSERT INTO #Config ([Name], [Value])
VALUES('CleanupTimeLog', @CleanupTimeLog)

INSERT INTO #Config ([Name], [Value])
VALUES('ScriptVersion', @ScriptVersion)

INSERT INTO #Config ([Name], [Value])
VALUES('UseDiffs', @UseDiffs)

INSERT INTO #Config ([Name], [Value])
VALUES('SystemTime', @SystemTime)

INSERT INTO #Config ([Name], [Value])
VALUES('FullTime', @FullTime)

INSERT INTO #Config ([Name], [Value])
VALUES('DiffTime', @DiffTime)

INSERT INTO #Config ([Name], [Value])
VALUES('LogTime', @LogTime)

INSERT INTO #Config ([Name], [Value])
SELECT 'LogInterval', (SELECT CASE
                    WHEN @LogInterval = 'Seconds'
                    THEN 2
                    WHEN @LogInterval = 'Minutes'
                    THEN 4
                    WHEN @LogInterval = 'Hours'
                    THEN 8
          END)

INSERT INTO #Config ([Name], [Value])
VALUES('LogFrequency', @LogFrequency)

INSERT INTO #Config ([Name], [Value])
VALUES('IntegrityTime', @IntegrityTime)
...
INSERT INTO #Config ([Name], [Value])
SELECT 'FullDay', (SELECT CASE
                    WHEN @FullDay = 'Sunday'
                    THEN 1
                    WHEN @FullDay = 'Monday'
                    THEN 2
                    WHEN @FullDay = 'Tuesday'
                    THEN 4
                    WHEN @FullDay = 'Wednesday'
                    THEN 8
                    WHEN @FullDay = 'Thursday'
                    THEN 16
                    WHEN @FullDay = 'Friday'
                    THEN 32
                    WHEN @FullDay = 'Saturday'
                    THEN 64
          END)

INSERT INTO #Config ([Name], [Value])
SELECT 'IntegrityDay', (SELECT CASE
                    WHEN @IntegrityDay = 'Sunday'
                    THEN 1
                    WHEN @IntegrityDay = 'Monday'
                    THEN 2
                    WHEN @IntegrityDay = 'Tuesday'
                    THEN 4
                    WHEN @IntegrityDay = 'Wednesday'
                    THEN 8
                    WHEN @IntegrityDay = 'Thursday'
                    THEN 16
                    WHEN @IntegrityDay = 'Friday'
                    THEN 32
                    WHEN @IntegrityDay = 'Saturday'
                    THEN 64
          END)

INSERT INTO #Config ([Name], [Value])
SELECT 'IndexDay', (SELECT CASE
                    WHEN @IndexDay = 'Sunday'
                    THEN 1
                    WHEN @IndexDay = 'Monday'
                    THEN 2
                    WHEN @IndexDay = 'Tuesday'
                    THEN 4
                    WHEN @IndexDay = 'Wednesday'
                    THEN 8
                    WHEN @IndexDay = 'Thursday'
                    THEN 16
                    WHEN @IndexDay = 'Friday'
                    THEN 32
                    WHEN @IndexDay = 'Saturday'
                    THEN 64
          END)
...
BEGIN
  ...
  ----------------------------------------------------------------------------------------------------
  --// Source: https://ola.hallengren.com             07 Oct 2016                                  //--
  ----------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  DECLARE @ToCompressBackups nvarchar(max)
  DECLARE @ScriptVersion VARCHAR(5)
  DECLARE @UseDiffs VARCHAR(1)
  DECLARE @DiffDay int

  DECLARE @SystemTime int
  DECLARE @FullDay int
  DECLARE @FullTime int
  DECLARE @DiffTime int
  DECLARE @LogTime int
  DECLARE @LogInterval int
  DECLARE @LogFrequency int
  DECLARE @IntegrityDay int
  DECLARE @IntegrityTime int
 ...
 --- WardyIT Solutions ---
  ...
  SELECT @CleanupTimeSystem = Value
  FROM #Config
  WHERE [Name] = 'CleanupTimeSystem'

 SELECT @CleanupTimeFull = Value
  FROM #Config
  WHERE [Name] = 'CleanupTimeFull'

  SELECT @CleanupTimeDiff = Value
  FROM #Config
  WHERE [Name] = 'CleanupTimeDiff'

 SELECT @CleanupTimeLog = Value
  FROM #Config
  WHERE [Name] = 'CleanupTimeLog'

  SELECT @ToCompressBackups = Value
  FROM #Config
  WHERE [Name] = 'ToCompressBackups'

  SELECT @ScriptVersion = Value
  FROM #Config
  WHERE [Name] = 'ScriptVersion'

  SELECT @UseDiffs = Value
  FROM #Config
  WHERE [Name] = 'UseDiffs'

---Scheudle section---

  SELECT @SystemTime = Value
  FROM #Config
  WHERE [Name] = 'SystemTime'

  SELECT @FullDay = Value
  FROM #Config
  WHERE [Name] = 'FullDay'

  SELECT @FullTime = Value
  FROM #Config
  WHERE [Name] = 'FullTime'

  SELECT @DiffTime = Value
  FROM #Config
  WHERE [Name] = 'DiffTime'
  ...
  SELECT @LogInterval = Value
  FROM #Config
  WHERE [Name] = 'LogInterval'
  
  SELECT @LogFrequency = Value
  FROM #Config
  WHERE [Name] = 'LogFrequency'

  SELECT @IntegrityDay = Value
  FROM #Config
  WHERE [Name] = 'IntegrityDay'

  SELECT @IntegrityTime = Value
  FROM #Config
  WHERE [Name] = 'IntegrityTime'

  SELECT @IndexDay = Value
  FROM #Config
  WHERE [Name] = 'IndexDay'

  SELECT @IndexTime = Value
  FROM #Config
  WHERE [Name] = 'IndexTime'

   SET @DiffDay = SUM(127 - @FullDay)
---Schedule End---
--- End ---
  ...
  SET @JobDescription01 = 'Performs a full backup of all system databases. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription02 = 'Performs a differential backup of all user databases. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription03 = 'Performs a full backup of all user databases. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription04 = 'Performs a transaction log backup of all user databases. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription05 = 'Performs an integrity check of all system databases. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription06 = 'Performs an integrity check of all user databases. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription07 = 'Performs index maintenance of all user databases, including updating statistics. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription08 = 'Performs a backup history cleanup. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription09 = 'Purges the job history that are outside a specific date range. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription10 = 'Performs a cleanup of all output files. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription11 = 'Removes logs from the default master.dbo.CommandLog table that are outside a specific date range. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription12 = 'Archives the current SQL Server log file and creates a new log file to store events. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription13 = 'Performs an integrity check of all user and system databases. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'
  SET @JobDescription14 = 'Performs weekly maintenance tasks for Command Log Cleanup, Cycle Error Log, Delete Backup History, Output File Cleanup and Purging the job history. WARDY IT SOLUTIONS ' + CONVERT(nvarchar(4),DATEPART(YY,GETDATE())) + ' (' + @ScriptVersion + ')'

  SET @JobName01 = 'ADMIN-Database Backup SYSTEM FULL'
  SET @JobCommand01 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = '+@CleanupTimeSystem+', @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobCommand02 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''DIFF'', @Verify = ''Y'', @CleanupTime = '+@CleanupTimeDiff+', @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobCommand03 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = '+@CleanupTimeFull+' , @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobCommand04 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''LOG'', @Verify = ''Y'', @CleanupTime = ' + @CleanupTimeLog + ', @Compress = ''' + @ToCompressBackups + ''', @ChangeBackupType = ''Y'', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
...
cmd /q /c "For /F "tokens=1 delims=" %v In (''ForFiles /P "' + @OutputFileDirectory + '" /m "ADMIN-Database Backup USER LOG*.txt" /d -7 2^>^&1'') do if EXIST "' + @OutputFileDirectory + '"\%v echo del "' + @OutputFileDirectory + '"\%v& del "' + @OutputFileDirectory + '"\%v"'
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName01, @freq_type = 4, @freq_interval = 1, @active_start_time = @SystemTime
  ...
  BEGIN
      ...
      EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName02, @freq_type = 8, @freq_interval = @DiffDay, @freq_recurrence_factor = 1, @active_start_time = @DiffTime
  ...
  BEGIN
    ...
    BEGIN
      ...
      EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName03, @freq_type = 8, @freq_interval = @FullDay, @freq_recurrence_factor = 1, @active_start_time = @FullTime
    ...
    BEGIN
      EXECUTE msdb.dbo.sp_add_job @job_name = @JobName03, @description = @JobDescription03, @category_name = @JobCategory, @owner_login_name = @JobOwner, @enabled = 0
      EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName03, @step_name = @JobName03, @subsystem = 'CMDEXEC', @command = @JobCommand03, @output_file_name = @OutputFile03
      EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName03, @freq_type = 4, @freq_interval = 1, @active_start_time = @FullTime
      EXECUTE msdb.dbo.sp_attach_schedule @job_name = @JobName03, @schedule_name = @JobName03
      EXECUTE msdb.dbo.sp_add_jobserver @job_name = @JobName03
    END
  END
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName04, @freq_type = 4, @freq_interval = 1, @freq_subday_type = @LogInterval, @freq_subday_interval = @LogFrequency, @active_start_time = @LogTime, @active_end_time = 235959
 ...
 -- END
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName07, @freq_type = 8, @freq_interval = @IndexDay, @freq_recurrence_factor = 1, @active_start_time = @IndexTime
 ...
 -- END
  ...
  EXECUTE msdb.dbo.sp_add_schedule @schedule_name = @JobName13, @freq_type = 8, @freq_interval = @IntegrityDay, @freq_recurrence_factor = 1, @active_start_time = @IntegrityTime
